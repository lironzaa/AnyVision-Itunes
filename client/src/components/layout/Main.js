    ...
    render() {
        ...
        const searchItems = songs.length ? (
            songs.map((item, index) => {
                return (
                    <div onClick={() => this.onGetItem(item)} className="col-lg-2 col-md-6 col-sm-10 card mb-5"
                         key={index}>
                        <img className="card-img-top" src={item.artworkUrl100} alt={item.trackName}/>
                        <div className="card-body">
                            ...
                            <p className="card-text"><span
                                className="font-weight-bold">Artist :</span> {item.artistName}</p>
                            <p className="card-text"><span
                                className="font-weight-bold">Album :</span> {item.collectionName}</p>
                        </div>
                    </div>
                )
            })
        ) : resultCount === null ? <div className="mx-auto">Please search for a song/movie</div> :
            <div className="mx-auto">No results for this search</div>;
                        ...
                        <form onSubmit={this.onSubmit} noValidate>
                            <div className="form-group">
                                <input type="text" value={this.state.searchedQuery}
                                       ...
                                       placeholder="Search" name="searchedQuery"/>
                            </div>
                            <input type="button" value="Show Top 10 Searched" className="btn btn-success btn-block mt-4"
                                   onClick={this.onGetTop10}/>
